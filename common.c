...
void
undo_init(int maxdepth, struct _undo *undo)
{
  if (!undo) return;
...
undo_prune(struct _undo *undo)
{
  ...
  if (undo->stack_depth > undo->max_stack_depth) {
    ...
    if (tmp)
      while (tmp->next && --depth) tmp = tmp->next;
...
void
undo_push(struct td_surf *surface, struct _undo *undo)
{
...
int
undo_pop(struct td_surf *surface, struct _undo *undo)
{
  if (undo) {
...
int
common_set_tile_position(int mx, int my, struct td_surf *surface)
{
  int x,y,i;
  struct td_surf *root_surf, *child_surf;

  if (surface == NULL) return FAILED;
  if (surface->parent == NULL && surface->child == NULL) return FAILED;
...
int
common_compute_tile_position(struct td_surf *surface)
{
  return common_set_tile_position(-1, -1, surface);
}

int
common_copy_data_to_child(struct td_surf *surface)
{
  int a, b, i, n, p;
  struct td_surf *root_surf, *child_surf;
...
int
common_copy_data_to_parent(struct td_surf *surface)
{
  int a, b, i, n, p;

  if (surface == NULL) return FAILED;
  if (surface->parent == NULL) return FAILED;
...
void
common_getpixel(int x, int y, struct td_color *c, struct td_surf *surface)
{
...
void
_common_putpixel(int x, int y, struct td_color *c, struct td_surf *surface)
{
...
void
_common_putpixel_negate(int x, int y, struct td_surf *surface)
{
  int i = (y*surface->width + x)<<2;
  if (surface->viewmode == VIEW_MODE_RGBA) {
...
void
_common_putpixel_view(int x, int y, struct td_color *c, struct td_surf *out)
{
...
void
_common_putpixel_view_negate(int x, int y, struct td_surf *out)
{
  int ip = (y*out->width*out->zoom + x)*3;
...
void
_common_putpixel_zoomedview_negate(int x, int y, struct td_surf *surface)
{
  unsigned char *d;
  int z, a, b, dx, dy, q;
...
void
_common_putpixel_zoomedview(int x, int y, struct td_color *c, struct td_surf *surface)
{
  unsigned char *d;
  int z, a, b, dx, dy;
...
void
common_putpixel_setmode(int putpix)
{
  common_putpix_mode = putpix;
}

void
common_putpixel(int x, int y, struct td_color *col, struct td_surf *surface)
{
  if (x < 0 || x >= surface->width || y < 0 || y >= surface->height) return;

  switch (common_putpix_mode) {
  default:
  case PUTPIX_NORMAL: _common_putpixel(x,y, col, surface);	break;
  case PUTPIX_NEGATE: _common_putpixel_negate(x,y, surface);	break;
  case PUTPIX_NORMAL|PUTPIX_VIEW: _common_putpixel_view(x,y, col, surface);	break;
  case PUTPIX_NEGATE|PUTPIX_VIEW: _common_putpixel_view_negate(x,y, surface);	break;
  case PUTPIX_NORMAL|PUTPIX_ZOOM|PUTPIX_VIEW: /* Zoom implies View */
  case PUTPIX_NORMAL|PUTPIX_ZOOM: _common_putpixel_zoomedview(x,y, col, surface);	break;
  case PUTPIX_NEGATE|PUTPIX_ZOOM|PUTPIX_VIEW: /* Zoom implies View */
  case PUTPIX_NEGATE|PUTPIX_ZOOM: _common_putpixel_zoomedview_negate(x,y, surface);	break;
  }
}

void
common_putrect(int x, int y, int x2, int y2, struct td_color *col, struct td_surf *surface, int filled)
{
  int dy,dx;
  int tmp;
...
void
common_putline(int x, int y, int x2, int y2, struct td_color *col, struct td_surf *surface)
{
...
void
_common_putcircle_filled(int center_x, int center_y, int rx, int ry, struct td_color *col, struct td_surf *surface)
{
...
void
_common_putcircle(int center_x, int center_y, int rx, int ry, struct td_color *col, struct td_surf *surface)
{
  /* intermediate terms to speed up loop */
  long t1 = rx*rx, t2 = t1<<1, t3 = t2<<1;
  long t4 = ry*ry, t5 = t4<<1, t6 = t5<<1;
...
void
common_putcircle(int center_x, int center_y, int rx, int ry, struct td_color *col, struct td_surf *surface, int filled)
{
  if (filled)
...
void
common_line_dotted_horiz(int y, struct td_surf *out)
{
  int x;
  int count = 0;
...
void
common_line_dotted_vert(int x, struct td_surf *out)
{
  int y;
  int count = 0;
  struct td_color col_black, col_white;
...
void
common_clear(struct td_color *col, struct td_surf *surface)
{
  int x,y;
...
int
common_editor_filter_3x3(float *filter, struct td_surf *surface)
{
...
void
common_filters_negate(struct td_surf *surface)
{
...
void
common_filters_build_alpha(struct td_color *col, struct td_surf *surface)
{
  unsigned int x, y, n;
...
void
common_mirror_lr(struct td_surf *surface)
{
...
void
common_mirror_tb(struct td_surf *surface)
{
...
void
common_rotate_right(struct td_surf *surface)
{
  unsigned int x, y;
  unsigned char *c;
...
void
common_rotate_left(struct td_surf *surface)
{
  unsigned int x, y;
  unsigned char *c;
...
void
common_mirror_and_blend_lin_x(struct td_surf *surface)
{
...
void
common_mirror_and_blend_lin_y(struct td_surf *surface)
{
  unsigned int x, y, a, b;
  float f, c, d, w, v;
...
void
common_mirror_and_blend_sto_x(struct td_surf *surface)
{
  unsigned int x, y, a, b;
  float c, d, w, v;
...
void
common_mirror_and_blend_sto_y(struct td_surf *surface)
{
  unsigned int x, y, a, b;
  float c, d, w, v;
...
void
common_erosion(struct td_color *col, struct td_surf *surface)
{
...
void
common_flip_x(struct td_surf *surface)
{
  unsigned char c;
  int x, y, a, b, n;
...
void
common_flip_y(struct td_surf *surface)
{
  unsigned char c;
  int x, y, a, b, n;
...
void
common_shift_left(struct td_surf *surface)
{
...
void
common_shift_right(struct td_surf *surface)
{
  unsigned char r, g, b, a;
  int x, y, n;
...
void
common_shift_up(struct td_surf *surface)
{
  unsigned char r, g, b, a;
  int x, y, n;
...
void
common_shift_down(struct td_surf *surface)
{
  unsigned char r, g, b, a;
  int x, y, n;
...
void
common_draw_blur_normal(unsigned int x, unsigned int y, struct td_surf *surface)
{
  unsigned i, a, b;
  float cr, cg, cb, ca;
...
void
common_draw_blur_gaussian(unsigned int x, unsigned int y, struct td_surf *surface)
{
  unsigned i, a, b, n;
  float cr, cg, cb, ca;
...
void
common_draw_fill(int x, int y, struct td_color *col, struct td_surf *surface)
{
  int i = (y*surface->width + x) * 4;
...
void
_common_aggressive_floodfill(int x, int y)
{
  int i, n, dR, dG, dB, dA;
...
void
common_draw_aggressive_fill(int x, int y, struct td_color *col, struct td_surf *surface)
{
  int i = (y*surface->width + x) * 4;
...
void
_common_gradient_floodfill(int x, int y, int oR, int oG, int oB, int oA)
{
  int i, n, dR, dG, dB, dA;
...
void
common_draw_gradient_fill(int x, int y, struct td_color *col, struct td_surf *surface)
{
  int i = (y*surface->width + x) * 4;
...
void
common_resize_data(int dx, int dy, struct td_color *col, struct td_surf *surface)
{
  unsigned char *m;
  int i, ox, oy, x, y;
